    ...
    def run(self):
        while True:
            ...
            if not tbd_url:
                ...
                self.counter.get_report()  # generate the report when its done
                                           # also can be modified to get report for
                                           # every n iterations in this while loop
                break
            resp = download(tbd_url, self.config, self.logger)
            self.logger.info(
                f"Downloaded {tbd_url}, status <{resp.status}>, "
                f"using cache {self.config.cache_server}.")
            scraped_urls = scraper.scraper(tbd_url, resp, self.counter)
